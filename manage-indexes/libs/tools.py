...
def splunkacs_delete_ephemeral_token(stack, tokenacs, tokenid, proxy_dict):
    ...
    # submit
    submit_url = "https://admin.splunk.com/%s/adminconfig/v2/tokens/" % stack
    submit_url = submit_url + str(tokenid)

    # run
    session = requests.Session()

    try:
        response = session.delete(submit_url, headers=headers, verify=True, proxies=proxy_dict)
        if response.status_code not in (200, 201, 204):
            logging.error("function=splunkacs_delete_ephemeral_token, Splunk ACS call has failed, url={},  HTTP Error={}, content={}".format(submit_url, response.status_code, response.text))
            raise Exception("Splunk ACS call has failed, url=\"{}\", response=\"{}\"".format(submit_url, response.text))
        else:
            logging.debug("Splunk ACS call was successful, url=\"{}\", response=\"{}\"".format(submit_url, response.text))

    except Exception as e:
        logging.error("function=splunkacs_delete_ephemeral_token, Splunk ACS call has failed, url=\"{}\", exception=\"{}\"".format(submit_url, e))
        raise Exception("Splunk ACS call has failed, url=\"{}\", exception=\"{}\"".format(submit_url, e))
