...
if args.mode:
    ...
    if not mode in ("live", "simulation"):
        logging.error(
            'invalid value mode="{}", valid options are simulation to show what we would do, live to proceed with ACS deployments accordingly'
        )
        sys.exit(1)
else:
    mode = "simulation"
...
if create_token:
    ...
    logging.error(
        "cannot request mode=live without the necessary information to connect to Splunk ACS"
    )
    sys.exit(1)
...
if userappinspect and passappinspect:
    ...
    try:
        ...
        logging.error(
            'failed to login to Appinspect, cannot continue, exception="{}"'.format(
                str(e)
            )
        )
        sys.exit(1)
...
try:
    ...
    logging.error('failed to request Splunk ACS API, exception="{}"'.format(str(e)))
    raise Exception('failed to request Splunk ACSAPI, exception="{}"'.format(str(e)))

# Start message
logging.info(
    '********** starting program massdeploy.py with appdir="{}", target_stack="{}" **********'.format(
        appdir, stack
    )
)
...
for appinfo in splunk_apps_dict:
    ...
    try:
        ...
        logging.debug(
            'app="{}" has no version information, will be ignored'.format(
                appinfo.get("name")
            )
        )
    ...
    try:
        ...
        logging.debug(
            'app="{}" has no build information, will be ignored'.format(
                appinfo.get("name")
            )
        )
                ...
                with cd(localappdir):
                    if os.path.isfile("version.txt"):
                        with open("version.txt") as f:
                            ...
                            logging.debug(
                                'app="{}", version="{}"'.format(appname, appversion)
                            )
                    ...
                    if os.path.isfile("build.txt"):
                        with open("build.txt") as f:
                            appbuild = f.readline()
                            logging.debug(
                                'app="{}", build="{}"'.format(appname, appbuild)
                            )
                    ...
                    if os.path.isfile(tgz[0]):
                        apparchive = tgz[0]
                        logging.debug(
                            'app="{}", apparchive="{}"'.format(appname, apparchive)
                        )
...
if len(apps_local_list) == 0:
    ...
    logging.info(
        '********** ending program massdeploy.py target_stack="{}" **********'.format(
            stack
        )
    )
    sys.exit(0)
...
for app in apps_local_list:
    ...
    logging.debug(
        'handling local app="{}", app_info="{}"'.format(
            app, json.dumps(apps_local_dict[app], indent=2)
        )
    )
    ...
    if not app in remote_apps_list:
        ...
        logging.info(
            'app="{}" is not installed yet on the target stack="{}", proceeding to its deployment now'.format(
                app, stack
            )
        )
    ...
    # uc2: app is deployed but out of date
        ...
        logging.debug(
            'app="{}" is installed on target stack="{}", checking if it is up to date'.format(
                app, stack
            )
        )
        ...
        if target_appversion and target_appbuild:
            ...
            if str(target_appversion) == str(remote_appversion) and int(
                target_appbuild
            ) == int(remote_appbuild):
                logging.info(
                    'app="{}", app is up to date, local_version="{}", remote_version="{}", local_build="{}", remote_build="{}", nothing to do.'.format(
                        app,
                        target_appversion,
                        remote_appversion,
                        target_appbuild,
                ...
                logging.info(
                    'app="{}", detected mistmatch, local_version="{}", remote_version="{}", local_build="{}", remote_build="{}", deployment will be requested.'.format(
                        app,
                        target_appversion,
                        remote_appversion,
                        target_appbuild,
        ...
        # if we only have a version
            ...
            if str(target_appversion) == str(remote_appversion):
                logging.info(
                    'app="{}", app is up to date, local_version="{}", remote_version="{}", nothing to do.'.format(
                        app, target_appversion, remote_appversion
                    )
                )

            else:
                logging.info(
                    'app="{}", detected mistmatch, local_version="{}", remote_version="{}", deployment will be requested.'.format(
                        app, target_appversion, remote_appversion
                    )
                )
                deployment_reason = "out_of_date"
    ...
    if deployment_reason and mode == "simulation":
        logging.info(
            "simulation mode terminated, to proceed with deployment effectively, run this command with --mode=live"
        )

    elif deployment_reason:
        logging.info(
            'fasten your belt, Splunk ACS deployment starting now for app="{}", deployemnt_reason="{}"'.format(
                app, deployment_reason
            )
        )
        ...
        if count_failure == 0 and count_error == 0:
            logging.info(
                'Appinspect request_id="{}" was successfully vetted, summary="{}"'.format(
                    request_id, json.dumps(appinspect_report_dict["summary"], indent=4)
                )
            )
                    ...
                    if status_acs in ("installed", "uploaded"):
                        ...
                        logging.error(
                            'Splunk ACS deployment of app="{}" has failed, summary="{}"'.format(
                                file_name, json.dumps(splunkacs_response, indent=4)
                            )
                        )
                        raise ValueError(
                            'Splunk ACS deployment of app="{}" has failed, summary="{}"'.format(
...
#
logging.info(
    '********** ending program massdeploy.py target_stack="{}" **********'.format(stack)
)
