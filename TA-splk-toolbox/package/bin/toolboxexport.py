...
# set logging
filehandler = logging.FileHandler(splunkhome + "/var/log/splunk/toolbox_export.log", 'a')
...
class ToolboxExport(GeneratingCommand):
    ...
    account = Option(
        ...
        **Description:** Mandatory, the account''',
        require=True, default=None, validate=validators.Match("account", r".*"))

    remote_account = Option(
        doc='''
        **Syntax:** **The remote_account=****
        **Description:** Mandatory for sc_test and live, the remote_account''',
        require=False, default=None, validate=validators.Match("account", r".*"))
    ...
    mode = Option(
        ...
        **Description:** Optional, valid options are test | live. test will verify the connectivity, live runs the actual REST call''',
        require=False, default='live', validate=validators.Match("mode", r"^(?:test|sc_test|live)$"))
    ...
    def generate(self, **kwargs):
        ...
        if self:
            ...
            if not isfound:
                ...
                response = {
                    'action': 'failure',
                    'response': 'The account=\"{}\" specified does not exist in this system.'.format(self.account), 
                }
                ...
                # else get the bearer token stored encrypted
                    ...
                    credential_name = str(credential_realm) + ":" + str(self.account) + "``"
            ...
            if not bearer_token:
                ...
                if self.mode == 'sc_test' or self.mode == 'live':
                    if not self.remote_account or self.remote_account == 'None':
                        raise ValueError('The value for remote_account must be provided')
                ...
                if self.mode == 'test':
                    ...
                    logging.info("Establishing connection to host=\"{}\" on port=\"{}\" for Splunk remote account=\"{}\"".format(parsed_url.hostname, parsed_url.port, self.account))
                ...
                #
                    ...
                    # set url
                    url = str(splunk_url) + '/services/toolbox/v1/import/test_sc_connectivity'

                    try:

                        response = requests.post(url, headers={'Authorization': header}, data=json.dumps({'account': self.remote_account}), proxies=proxy_dict,
                                                    verify=False, timeout=30)
                    ...
                    try:
                        ...
                        response = requests.post(url, headers={'Authorization': header}, data=json.dumps(post_data), proxies=proxy_dict,
                                                    verify=False, timeout=30)
